...
def simple_detector(imageName):    
    ...
    # yen
    threshold = threshold_triangle(image)   #返回一个阈值
...
label_map = dict(zip([i for i in os.listdir('../data/train') if not i=='.DS_Store'], range(8)))

for d in os.listdir('../data/train'):
    print("Processing {}...".format(d))
    if d == '.DS_Store':
        continue
    for f in os.listdir(os.path.join('../data/train', d)):
        labels.append(label_map[d])
        o_im = load_img(os.path.join('../data/train', d, f))
...
model.save("threshold_triangle.h5")
...
for fname in os.listdir('../data/test_stg1'):
    if fname == '.DS_Store':
        continue
    test_im_names.append(fname)
    o_im = load_img(os.path.join('../data/test_stg1', fname))
    im=simple_detector(o_im)  
    im_data = img_to_array(im)
    im_data.resize(100, 100, 3)
